                               ...
                               const VrmlData_Scene& theScene)
{
  ...
  aScene.myMutex.Lock();
  aScene.myCurrentIndent = aScene.isX3D() ? 2 : 0;
  ...
  if (theScene.isX3D()) {
    ...
    theOutput << "<!DOCTYPE X3D PUBLIC \"ISO//Web3D//DTD X3D 3.2//EN\" \"http://www.web3d.org/specifications/x3d-3.2.dtd\">\n";
    theOutput << "<X3D xmlns:xsd='http://www.w3.org/2001/XMLSchema-instance' profile='Interchange' version='3.2' xsd:noNamespaceSchemaLocation='http://www.web3d.org/specifications/x3d-3.2.xsd'>\n";
    theScene.WriteLine("<Scene DEF='scene'>", 0L, VrmlData_Node::GlobalIndent());
  }
  else {
  ...
  if (theScene.isX3D())
  {
    theScene.WriteLine("</Scene>", 0L, -VrmlData_Node::GlobalIndent());
    theOutput << "</X3D>\n";
  }
                                 ...
                                 const Standard_Size       theNbBlocks) const
{
  ...
  if (theNbBlocks && (IsDummyWrite() == Standard_False)) {
    if (isX3D()) {
      WriteLine (" ", 0L, 0, false, false);
    }
    if (VrmlData_Node::OK (aStatus,
                           WriteLine (thePrefix, isX3D() ? "='" : "[", 1, !isX3D(), !isX3D())))
    {
      ...
      for (Standard_Size iBlock = 0; iBlock < theNbBlocks; iBlock++) {
        ...
        switch (nVal) {
        case 1:
          Sprintf (buf, isX3D() ? "%d " : "%d,", arrVal[0]);
          break;
        case 2:
          Sprintf (buf, isX3D() ? "%d %d " : "%d,%d,", arrVal[0], arrVal[1]);
          break;
        case 3:
          Sprintf (buf, isX3D() ? "%d %d %d %d " :"%d,%d,%d,", arrVal[0], arrVal[1], arrVal[2]);
          break;
        case 4:
          Sprintf (buf, isX3D() ? "%d %d %d %d " :"%d,%d,%d,%d,",
          ...
          if (nVal > 0) {
            ...
            for (Standard_Integer i = 0; i < nVal; i++) {
              ...
              if ((ptr - &buf[0]) > (ptrdiff_t)aLineLimit) {
                  WriteLine(buf, 0L, 0, !isX3D(), !isX3D());
                ptr = &buf[0];
              }
            }
          }
        ...
        WriteLine (buf, iBlock < theNbBlocks-1 ?
                        (isX3D() ? "-1 " : "-1,") :
                        (isX3D() ? "-1" : "-1"), 0, !isX3D(), !isX3D());
      }
      if (aStatus == VrmlData_StatusOK)
        aStatus = WriteLine (isX3D() ? "'" : "]", 0L, -1, !isX3D(), !isX3D());
    }
  }
  return aStatus;
}
                                 ...
                                 const char             * thePostfix) const
{
  ...
  return WriteLine (buf, 0L, 0, !isX3D(), !isX3D());
}
...
VrmlData_ErrorStatus VrmlData_Scene::WriteLine
                                        (const char             * theLin0,
                                         ...
                                         const Standard_Boolean theToIndent,
                                         const Standard_Boolean theToCarriageReturn) const
{
...
VrmlData_ErrorStatus VrmlData_Scene::WriteDefUse(const Handle(VrmlData_Node)& theNode) const
{
  ...
  if (* nptr == '\0')
    ...
    if (myNamedNodesOut.Contains (theNode))
    {
      ...
      buf += theNode->Name();
      buf += "'";
      WriteLine (buf.ToCString(), 0L, 0, false, false);
      return VrmlData_Use;
    }
    else
    {
