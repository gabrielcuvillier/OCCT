...
void HandleVertices(std::shared_ptr<PlyData> theData,
    ...
    int i = 0;
    for (auto node: vec) {
        thePoly->InternalNodes().SetValue(i + 1, gp_Pnt(node.x, node.y, node.z));
        i++;
    }
}
...
void HandleTexCoords(std::shared_ptr<PlyData> theData,
    ...
    int i = 0;
    for (auto node: vec) {
        thePoly->InternalUVNodes().SetValue(i + 1, gp_Pnt2d(node.x, node.y));
        i++;
    }
}
...
void HandleFaces(std::shared_ptr<PlyData> theData,
    ...
    int i = 0;
    for (auto node: vec) {
        thePoly->InternalTriangles().SetValue(i + 1, Poly_Triangle(node.x + 1, node.y + 1, node.z + 1));
        i++;
    }
}
...
opencascade::handle<Poly_Triangulation> RWPly::ReadFile(
    ...
    Message_ProgressScope aRange(theProgress, "Read PLY File", 1.);
    ...
    auto progress_callback =
            [&aRange](const tinyply::ProgressCallbackInfo info) {
                aRange.Next(
