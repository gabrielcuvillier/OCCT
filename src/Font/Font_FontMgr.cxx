                      ...
                      signed long theFaceId = -1) // FT_Long
{
    ...
    if (aDataStream) {
        ...
        if (aDataStream) {
            ...
            if (aDataStream) {
                aFaceError = FT_New_Memory_Face(theFTLib->Instance(), file_data_buffer, (FT_Long) aFileSize, 0,
                                                &aFontFace);
            }
        }
        aDataStream.close();
...
Standard_Boolean Font_FontMgr::RegisterFont(const Handle(Font_SystemFont)& theFont,
    ...
    for (int anAspectIter = 0; anAspectIter < Font_FontAspect_NB; ++anAspectIter) {
        if (anOldFont->FontPath((Font_FontAspect) anAspectIter).IsEqual(
...
#else
    // Minimal Freetype build does not know how to read woff fonts
